...
export default function TweetComponent({
    ...
    const ownerData = tweet?.owner as User;
    const { avatar, fullName, userName } = ownerData || {};
    const [commentsPost, setCommentPosts] = React.useState<Tweet[]>([]);
    ...
    const [popupType, setPopupType] = React.useState<"edit" | "delete" | string>("edit");
    ...
    useEffect(() => {
        ...
        if (typeof idx === 'number' && typeof tweetsLength === 'number' && idx === tweetsLength - 1) {
    ...
    }, [tweet, user?._id, idx, tweetsLength, setLoading, ownerData?._id]);
